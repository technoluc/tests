...
$choice = $(Write-Host "[1]Pop-Cat `n[2]Patrick Star `n[3]Kirby `n[4]Sword Kirby `n[5]Bulbasaul `n[6]Minecraft Chest `n[7]Dachshund `n[9]Default " -ForegroundColor Yellow) + $(Write-Host "Choice: " -NoNewLine; Read-Host)
...
if ($choice -eq '2') {
    ...
    $url = $pstarempty
    $output = "$env:userprofile\icons\patrick-star-empty.ico"
    Invoke-WebRequest -Uri $url -OutFile ( New-Item -Path "$output" -Force )
    Write-Host "Downloading patrick Star Full Icon"
    $url = $pstarfull
    $output = "$env:userprofile\icons\patrick-star-full.ico"
    Write-Host "Setting Registry"
    Invoke-WebRequest -Uri $url -OutFile ( New-Item -Path "$output" -Force )
    Set-ItemProperty -Path "Registry::HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CLSID\{645FF040-5081-101B-9F08-00AA002F954E}\DefaultIcon" -Name "(Default)" -Value "$env:userprofile\icons\patrick-star-empty.ico,0"
    Set-ItemProperty -Path "Registry::HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CLSID\{645FF040-5081-101B-9F08-00AA002F954E}\DefaultIcon" -Name "full" -Value "$env:userprofile\icons\patrick-star-full.ico,0"
    Set-ItemProperty -Path "Registry::HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CLSID\{645FF040-5081-101B-9F08-00AA002F954E}\DefaultIcon" -Name "empty" -Value "$env:userprofile\icons\patrick-star-empty.ico,0"
}
